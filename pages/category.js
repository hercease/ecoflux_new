...
export default function Category({data, user}) {
	...
	const handleChange = (event, value) => {
		setPage(value);
		router.push({ pathname: '/category', query: { page: value, search : search } })
	};
	...
	function Search(e){
		...
		setSearch(s)
		router.push({ pathname: '/category', query: { page : page, search: s } })
	}
  ...
  return (
	...
	<div className="container-fluid bg-secondary py-5">
		<div className="d-flex flex-column align-items-center justify-content-center">
			<h3 className="fw-bold text-white text-capitalize mt-4 p-4">All Category</h3>
		</div>
	</div>
	...
	<div className="container-fluid bg-light py-3 mb-3">
		...
		<div className="col-md-4 float-end">
			<input type="search" onChange={Search} placeholder="Search category" className="border-1 text-black form-control" />
		</div>
		</div>
	</div>
	...
	<div className="container-fluid mb-md-0">
		<div className="container">
			...
			<div className="row">
				...
				<div key={key} className="col-6 col-md-3 col-lg-3 col-sm-4 mb-3">
					...
					<div className="grid_item">
						<figure>
							<a onClick={() => router.push("/products?search="+item.category)}>
								<Image className="figure-img img-fluid" loader={() => `/${item.prod_image}`} src={`/${item.prod_image}`} height={300} width={300} />
							</a>
						</figure>
						<div className="position-absolute top-0 start-0"><span className="badge bg-light rounded-pill text-dark ms-2">{`${item.prod_count} Products`}</span></div>
						<a onClick={() => router.push("/products?search="+item.category)}>
							<h3> {item.category} </h3>
						</a>
					</div>
			...
			<div className="d-flex justify-content-center mb-3">
				<Pagination count={data?.length > 0 ? data[0].total_page : 0} page={page} onChange={handleChange} shape="rounded" color="info" />
			</div>
		</div>
	</div>
