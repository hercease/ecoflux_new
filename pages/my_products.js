...
export default function Myproducts({ data }){
	...
	const handleChange = (event, value) => {
		setPage(value);
		router.push({ pathname: '/my_products', query: { page: value, search : search } })
	};
	...
	function Search(e){
		...
		setSearch(s)
		router.push({ pathname: '/my_products', query: { page : page, search: s } })
	}
	
	async function Deleteproduct(value){
       await axios.post(`${process.env.dbname}/ecoflux/api/deleteproduct/`, { id : value }).then(function(response){
			if(response.data.status==1){
				setState({ ...state, open: true, backdrop:false, confirmopen: false, err_message: response.data.message });
				router.push("/my_products");
			}
        });
    }
	
	const [state, setState] = useState({ backdrop : false, confirmopen: false, id : 0, err_message: '',  open: false, vertical: 'top', severity:'success', horizontal: 'right',  });
 
	const { backdrop, id, vertical, horizontal, err_message, severity, open, confirmopen } = state;
	
const myLoader = ({ src, width, quality }) => {
  return `${process.env.dbname}/images/${src}?w=${width}&q=${quality || 75}`
}
...
function Opencollapse(){
  ...
  return (
	...
	<Head>
		<meta charSet="utf-8" />
		<title>Ecoflux | My Products</title>
	...
	<Dialog
        ...
        <DialogActions>
          <Button variant="contained" color="primary"  onClick={() => setState({ ...state, confirmopen: false }) }>No</Button>
          <Button variant="contained" color="success"  onClick={function(event){ setState({ ...state, confirmopen: false, backdrop: true }); Deleteproduct(id) }}>Yes</Button>
        </DialogActions>
    </Dialog>
	...
	<div className="container-fluid bg-secondary py-5">
		<div className="d-flex flex-column align-items-center justify-content-center">
			<h3 className="fw-bold text-white text-capitalize mt-4 p-4">My Products</h3>
		</div>
	</div>
	...
	<div className="container-fluid bg-light py-3">
		...
		<div className="col-md-4 float-end">
			<input type="search" onChange={Search} placeholder="Search your Products" className="border-1 text-black form-control" />
		</div>
		</div>
	</div>
			...
			<div className="row">
				...
				<div key={key} className="col-6 col-md-3 col-lg-3 col-sm-4 mb-3">
					...
					<div className="grid_item">
						<div style={{ zIndex: 999 }} className="position-absolute top-0 end-0">
							<span style= {{ width:'35px', height:'35px', borderRadius: '14px', padding: '4px' }} className="text-danger"><DeleteIcon onClick={function(event){ setState({ ...state, confirmopen: true, id:item.prod_id }) }} /></span>
						</div>
						<figure>
							<a onClick={() => router.push("/product/"+item.prod_id)}>
								
								<div style={{ width: 300, height: 300, position: 'relative', margin:"0px 0px 0px 0px" }}>
		...
		<div className="container">
		  <div style={{ height: "50vh" }} className="row justify-content-center align-items-center">
			<div className="col-md-12">
				<div className="card">
					<div className="card-header bg-light">Ooops</div>
					<div className="card-body">
						You do not have any product in your catalogue yet, kindly click the button below to get started
						<p className="text-center"><Link href="/upload_product"><ColorButton variant="contained">Get started</ColorButton></Link></p>
					</div>
				</div>
			</div>
		  </div>
			...
			<div className="d-flex justify-content-center mb-3">
				<Pagination count={data?.length > 0 ? data[0].total_page : 0} page={page} onChange={handleChange} shape="rounded" color="primary" />
			</div>
		</div>
	</div>
...
if(!cookies){
  ...
  try{
	...
	const [fetchproductsRes] = await Promise.all([
    		axios.get(`${process.env.dbname}/ecoflux/api/fetchmyproduct/${page}/${search}/${cookies}`)
  	]);
